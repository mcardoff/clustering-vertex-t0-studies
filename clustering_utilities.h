...
namespace myutl {
  ...
  static TF1* create_dgaus_fit(TH1D *hist, bool fixbkg) {
    ...
    fit->SetParLimits(3, 5, 1.E6); // can only be positive
    if (fixbkg)
  ...
  struct PlotObj {
	...
	if (first) {
      ...
      // Draw Efficiencies
      first = true;
      double eff_ymin = 0.0, eff_ymax = 1.4;
      for (auto score : enum_vec) {
	auto eff = efficiency.at(score).get();
	auto total = this->eff_total.get();
	if (first) {
	  eff->Draw("AP");
	  ...
	  double count_max = total->GetMaximum();
	  double scale = (eff_ymax - eff_ymin) / count_max;
	  total->Scale(scale);
	  total->Draw("HIST SAME");
	  double xright = gPad->GetUxmax();
	  TGaxis* axis = new TGaxis(xright, eff_ymin, xright, eff_ymax, 0, count_max, 510, "+L");
	  axis->SetLineColor(colors[8]);
	  axis->SetLabelColor(colors[8]);
	  axis->SetTitle("Frequency");
	  axis->SetTitleColor(colors[8]);
	  axis->Draw();
	  first = false;
	} else
	  eff->Draw("P SAME");
      ...
      efflegend->AddEntry(eff_total.get(),this->xtitle);
  ...
  static void process_event_data(
    ...
    if (cluster.size() != 0)
      chosen = chooseCluster(cluster, branch);
    ...
    // bool eventquery = (not passEfficiency(chosen[ScoreType::TRKPT], branch)) and cluster.size() != 0;
	...
	continue;
      ...
      if (cluster.size() == 0 and score != HGTD)
	continue;
